...
void CC1101_init() { /* CC1101 - Set default´s */
  ...
  if (chipVersion == 0x14 || chipVersion == 0x04 || chipVersion == 0x03 || chipVersion == 0x05 || chipVersion == 0x07 || chipVersion == 0x17) {  // found CC110x
    ...
    CC1101_found = true;
#ifdef debug_cc110x
    Serial.print(F("[DB] CC110x_PARTNUM                  ")); Serial.println(CC1101_readReg(CC1101_PARTNUM, READ_BURST), HEX);        // PARTNUM – Chip ID
    Serial.print(F("[DB] CC110x_VERSION                  ")); Serial.println(chipVersion, HEX);                                       // VERSION – Chip ID
    Serial.print(F("[DB] CC110x_FREQEST                  ")); Serial.println(CC1101_readReg(CC1101_FREQEST, READ_BURST), HEX);        // FREQEST – Frequency Offset Estimate from Demodulator
    Serial.print(F("[DB] CC110x_LQI                      ")); Serial.println(CC1101_readReg(CC1101_LQI, READ_BURST), HEX);            // LQI – Demodulator Estimate for Link Quality
    Serial.print(F("[DB] CC110x_RSSI                     ")); Serial.println(CC1101_readReg(CC1101_RSSI, READ_BURST), HEX);           // RSSI – Received Signal Strength Indication
    Serial.print(F("[DB] CC110x_MARCSTATE                ")); Serial.println(CC1101_readReg(CC1101_MARCSTATE, READ_BURST), HEX);      // MARCSTATE – Main Radio Control State Machine State
    Serial.print(F("[DB] CC110x_WORTIME1                 ")); Serial.println(CC1101_readReg(CC1101_WORTIME1, READ_BURST), HEX);       // WORTIME1 – High Byte of WOR Time
    Serial.print(F("[DB] CC110x_WORTIME0                 ")); Serial.println(CC1101_readReg(CC1101_WORTIME0, READ_BURST), HEX);       // WORTIME0 – Low Byte of WOR Time
    Serial.print(F("[DB] CC110x_PKTSTATUS                ")); Serial.println(CC1101_readReg(CC1101_PKTSTATUS, READ_BURST), HEX);      // PKTSTATUS - Current GDOx Status and Packet Status
    Serial.print(F("[DB] CC110x_VCO_VC_DAC               ")); Serial.println(CC1101_readReg(CC1101_VCO_VC_DAC, READ_BURST), HEX);     // VCO_VC_DAC - Current Setting from PLL Calibration Module
    Serial.print(F("[DB] CC110x_TXBYTES                  ")); Serial.println(CC1101_readReg(CC1101_TXBYTES, READ_BURST), HEX);        // TXBYTES - Underflow and Number of Bytes
    Serial.print(F("[DB] CC110x_RXBYTES                  ")); Serial.println(CC1101_readReg(CC1101_RXBYTES, READ_BURST), HEX);        // RXBYTES - Underflow and Number of Bytes
    Serial.print(F("[DB] CC110x_RCCTRL1_STATUS           ")); Serial.println(CC1101_readReg(CC1101_RCCTRL1_STATUS, READ_BURST), HEX); // CC1101_RCCTRL1_STATUS - Last RC Oscillator Calibration Result
    Serial.print(F("[DB] CC110x_RCCTRL0_STATUS           ")); Serial.println(CC1101_readReg(CC1101_RCCTRL0_STATUS, READ_BURST), HEX); // CC1101_RCCTRL0_STATUS - Last RC Oscillator Calibration Result
    Serial.print(F("[DB] CC110x_available_modes          ")); Serial.println(RegistersCntMax);                                        // Number of compiled register modes
    ...
    Serial.print(F("[DB] CC110x, number of registers     ")); Serial.println(RegistersCntMax);
#endif
    ...
    if (EEPROMread(EEPROM_ADDR_FW1) == Prog_Ident1 && EEPROMread(EEPROM_ADDR_FW2) == Prog_Ident2 && chk == chk_comp) {  // EEPROM OK, chk OK
      ...
      Serial.print(F("[DB] read activated mode from EEPROM ")); Serial.println(activated_mode_nr);
#endif
      ...
      if (activated_mode_nr > 0 && ToggleTime == 0) {
        ...
        Serial.print(F("[DB] write activated_mode_nr ")); Serial.print(activated_mode_nr);
...
#ifdef debug_cc110x
        if (i == 254) {
          Serial.println("[DB] CC1101_init, ERROR read CC1101_MARCSTATE, READ_BURST !");
        }
#endif
      }
      // Serial.println(CC1101_readReg(CC1101_MARCSTATE, READ_BURST), HEX); // MARCSTATE – Main Radio Control State Machine State (1 = Idle)
      ...
      Serial.print(F("[DB] activated_mode_nr = "));
