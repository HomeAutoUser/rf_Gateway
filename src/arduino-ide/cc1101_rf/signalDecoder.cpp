...
void SignalDetectorClass::addData(const int8_t value) {
  if (message.addValue(value)) {
    ...
    printOut();
    DBG_PRINT(" addData oflow-> mstart="); DBG_PRINT(mstart);
...
inline void SignalDetectorClass::doDetect() {
  ...
  if (!valid) {
    ...
    rssiFhem = Chip_readRSSI(); // Read RSSI value from Register
  }
  ...
  if (fidx >= 0) {
    ...
    if (patternLen == maxNumPattern) {
      ...
      for (uint8_t i = messageLen - 1 ; histo[pattern_pos] > 0 && messageLen > 0; --i) {
        //N:\ESP32\Arduino\ESP32_SX1231_StarterKit\ESP32_SX1231_SK_V0_040_opt\signalDecoder.cpp: In member function 'void SignalDetectorClass::doDetect()':
        //N:\ESP32\Arduino\ESP32_SX1231_StarterKit\ESP32_SX1231_SK_V0_040_opt\signalDecoder.cpp:199:43: warning: comparison is always true due to limited range of data type [-Wtype-limits]
        //  199 |       for (uint8_t i = messageLen - 1 ; i >= 0 && histo[pattern_pos] > 0 && messageLen > 0; --i) {
        //      |                                         ~~^~~~
...
void SignalDetectorClass::processMessage() {
  ...
  if (mcDetected == true || messageLen >= minMessageLen) {
    ...
    if (state == syncfound && messageLen >= minMessageLen) { // MS Messages mit clock / Sync Verhaeltnis pruefen
      ...
      if ((m_endfound && (mend - mstart) >= minMessageLen) || (!m_endfound && messageLen < maxMsgSize && (messageLen - mstart) >= minMessageLen)) {
...
#endif
        // ### ### MS output raw message data ### ###
        strRawMsg = char(2);      // MSG_START
        strRawMsg += F("MS;");     // MSG_START
        ...
        if ((messageLen - mend) >= minMessageLen && MsMoveCount > 0) {
          ...
          //␂MS;P1=489;P4=-2045;P5=-3971;P6=-8054;D=1616141414151515141514151414141414141414141515151415151414141414141414141414141415141415;CP=1;SP=6;R=239;O;    m2;␃
          strRawMsg += 'm';
          strRawMsg += MsMoveCount;
          strRawMsg += ';';
        }
        strRawMsg += char(3);      // MSG_END
        strRawMsg += char(10);     // LINE_FEED
        msgCount++;
        msgCountMode[ReceiveModeNr]++;
#ifdef CODE_AVR
        Serial.print(strRawMsg);
#elif CODE_ESP
        MSG_OUTPUTALL(strRawMsg);
        WebSocket_raw(strRawMsg.substring(1, strRawMsg.length() - 2)); // char(2) am Anfang und char(3), char(10) am Ende entfernen
#endif
        m_truncated = false;
        success = true;
...
MUOutput:
    if (success == false && MCenabled) {
      ...
      if (MCenabled) {
        ...
        if (mcdecoder == nullptr) {
          mcdecoder = new ManchesterpatternDecoder(this);
        }
        if (mcDetected == false) {
          mcdecoder->reset();
        ...
        if ((mcDetected || mcdecoder->isManchester())) { // Check if valid manchester pattern and try to decode
          ...
          if (mcdecoder->doDecode()) {
            digitalWriteFast(LED, HIGH); // LED on
            // ### ### MC output raw message data ### ###
            strRawMsg = char(2);      // MSG_START
            //␂    MC;LL=-2926;LH=2935;SL=-1472;SH=1525    ;D=AFF1FFA1;C=1476;L=32;R=0;␃
            strRawMsg += F("MC;LL="); // MSG_START
            strRawMsg += pattern[mcdecoder->longlow];
            strRawMsg += F(";LH=");
            strRawMsg += pattern[mcdecoder->longhigh];
            strRawMsg += F(";SL=");
            strRawMsg += pattern[mcdecoder->shortlow];
            strRawMsg += F(";SH=");
            strRawMsg += pattern[mcdecoder->shorthigh];
            strRawMsg += F(";D=");
            //␂MC;LL=-2926;LH=2935;SL=-1472;SH=1525    ;D=AFF1FFA1     ;C=1476;L=32;R=0;␃
            char cbuffer[3];
            uint8_t idx;
            ...
            strRawMsg += mcdecoder->nibble_to_HEX(mcdecoder->getMCByte(idx) >> 4 & 0xF);
            if (mcdecoder->ManchesterBits.valcount % 8 > 4 || mcdecoder->ManchesterBits.valcount % 8 == 0) {
              strRawMsg += mcdecoder->nibble_to_HEX(mcdecoder->getMCByte(idx) & 0xF);
            }
            strRawMsg += F(";C=");
            strRawMsg += mcdecoder->clock;
            strRawMsg += F(";L=");
            strRawMsg += mcdecoder->ManchesterBits.valcount;
            strRawMsg += ';';
            strRawMsg += F("R=");
            strRawMsg += rssiFhem;
            strRawMsg += ';';
    ...
    if (MUenabled && !mcDetected && state == clockfound && success == false && messageLen >= minMessageLen) {
      ...
      DBG_PRINT(" MU found: ");
#endif
      // ### ### MU output raw message data ### ###
      strRawMsg = char(2);       // MSG_START
      strRawMsg += F("MU;");     // MSG_START
      calcHisto();
      for (uint8_t idx = 0; idx < patternLen; idx++) {
        if (pattern[idx] == 0 || histo[idx] == 0) continue;
...
#if DEBUGDETECT >= 1
      DBG_PRINT("nothing to do, MUenabled=");
      DBG_PRINT(MUenabled);
      DBG_PRINT(", MSenabled=");
      DBG_PRINT(MSenabled);
      DBG_PRINT(", MCenabled=");
...
bool SignalDetectorClass::getSync()
{
  ...
  { // clock wurde bereits durch getclock bestimmt
    ...
    for (int8_t p = patternLen - 1; p >= 0; --p)  // Schleife fuer langen Syncpuls
    {
      ...
      // uint16_t syncabs = abs(pattern[p]); // warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
      if ((pattern[p] < 0) &&
...
#ifdef DEBUGDECODE
  char value = '\0';
  //char value = NULL'; // warning: converting to non-pointer type 'char' from NULL
#endif
